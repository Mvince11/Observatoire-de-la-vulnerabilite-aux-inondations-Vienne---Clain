...
map = {
  ...
  map.on('load', () => {
    ...
    const layersData = [
      { data: bassin_vienne, type: 'line', id: 'bassinvienneLayer', color: 'green' },
      { data: region, type: 'line', id: 'regionLayer', color: 'red' },
      { data: departement, type: 'line', id: 'departementLayer', color: '#a772b9', width: 2 },
      { data: epci, type: 'line', id: 'epciLayer', color: '#567ae3' },
    ...
    if (e.features.length > 0) {
        ...
        const formatNumber = (num) => {
            // Vérifie si la valeur est définie et convertible en nombre
            const parsedNum = parseInt(num, 10);
            return !isNaN(parsedNum) ? parsedNum.toLocaleString('fr-FR') : 'Non disponible';
        };
        const popupContent = `
            <hr><strong><center>Commune : ${feature.properties.nom || 'Non disponible'}</center></strong><hr>
            <strong>- Population :</strong> ${formatNumber(feature.properties.population) || 'Non disponible'}<br>
            <strong>- Nombre de personnes en zone fortement inondable :</strong> ${feature.properties.s12a || '0'}<br>
        `;

...
map = {
  ...
  map.on('load', () => {
    ...
    const layersData = [
      ...
      { data: zone_inondable, type: 'fill', id: 'zoneinondableLayer', color: '#b4c3ef' }
    ];
    ...
    layersData.forEach(layer => {
      ...
      if (layer.type === 'fill') {
        layerConfig.paint['fill-color'] = layer.color;
        layerConfig.paint['fill-opacity'] = 0.4;
      } else if (layer.type === 'line') {
        layerConfig.paint['line-color'] = layer.color;
      ...
      map.on('click', layer.id, (e) => {
        ...
        const popupContent = Object.keys(properties)
          .map(prop => `<strong>${prop}:</strong> ${properties[prop]}`)
          .join('<br>');
    ...
    mapLayersDiv.innerHTML = `
      <button id="toggleMapMenuButton" class="btn btn-light">
        <img src="Images/images/icons/layers-half.svg" alt="Icône SVG" width="25" height="25">
      </button>

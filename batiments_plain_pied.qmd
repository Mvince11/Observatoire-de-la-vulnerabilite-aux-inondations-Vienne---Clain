  ...
  map.on('load', () => {
    ...
    // Ajout des couches vectorielles
      ...
      map.on('click', layer.id, (e) => {
        ...
        ? feature.properties.population.toLocaleString('fr-FR') + ' hab'
        : 'Non disponible';
...
viewof slider = Inputs.range(
  [0, 230], 
  { value: 230, step: 10, label: "Population max en zone inondable" }
)
...
```{r}
#| label: tbl-s12a
#| tbl-cap: "Données de la variable s12a"
...
indicateur_s12a <- st_read("layers/indicateurs.geojson", quiet = TRUE) %>%
  mutate(s12a = ifelse(is.na(s12a), 0, s12a)) %>%  # Remplace NA par 0
  st_drop_geometry() %>%
